...
async function resolveCourseId(input) {
  ...
  if (input.courseCode) {
    const rows = await query('SELECT id FROM courses WHERE code = ? LIMIT 1', [input.courseCode]);
    if (rows.length) return rows[0].id;
  }

  if (input.course) {
    const rows = await query('SELECT id FROM courses WHERE title = ? LIMIT 1', [input.course]);
    if (rows.length) return rows[0].id;
  }
